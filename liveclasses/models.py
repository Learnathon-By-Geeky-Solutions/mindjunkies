...
class LiveClass(models.Model):
    ...
    meeting_id = models.CharField(max_length=50, unique=True, blank=True)  # Unique Jitsi meeting ID
    ...
    def save(self, *args, **kwargs):
        if not self.meeting_id:
            self.meeting_id = f"mindjunkies-{uuid.uuid4().hex[:10]}"  # Unique meeting ID
