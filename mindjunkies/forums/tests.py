...
class ForumViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.teacher = User.objects.create_user(username="teacheruser", password="teachpass")
    ...
    def test_comment_submission_view(self):
        self.client.login(username="testuser", password="testpass")
        response = self.client.post(
            reverse("comment_submission", kwargs={"course_slug": self.course.slug, "topic_slug": self.forum_topic.slug}),
            {"content": "New Comment"}
        )
        self.assertEqual(response.status_code, 302)  # Redirect on success
    ...
    def test_reply_submission_view(self):
        self.client.login(username="testuser", password="testpass")
        response = self.client.post(
            reverse("reply_submission", kwargs={"course_slug": self.course.slug, "topic_slug": self.forum_topic.slug, "comment_id": self.comment.id}),
            {"body": "New Reply"}
        )
        self.assertEqual(response.status_code, 302)  # Redirect on success
